	...
	*/
	class product
	{
		...
		public function insert_product($date,$files){
			...
			$dvt = mysqli_real_escape_string($this->db->link, $date['dvt']);
			...
			if($product_code ='' || $productName == "" || $productQuantity == "" || $dvt == "" || $category == "" || $brand == "" || $product_desc == "" || $price == "" || $type == "" || $file_name == ""){
				$alert = "<span class='error'>Các trường không được để trống</span>";
				return $alert;
			}else{
				move_uploaded_file($file_temp, $uploaded_image);

				$query = "INSERT INTO tbl_product(productName,product_code,product_remain,productQuantity,dvt,catId,brandId,product_desc,price,type,image) VALUES('$productName','$product_code','$productQuantity','$productQuantity','$dvt','$category','$brand','$product_desc','$price','$type','$unique_image') ";
